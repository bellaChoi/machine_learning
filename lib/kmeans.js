...
/**
 * prediction clusters to be allocated from the means already obtained
 * @param  {array} data
 * @param  {string} distanceType
 * @param  {array} means
 * @return {number} clusters index
 */
Kmeans.predictCluster = function (data, distanceType, means) {
    var distance = getDistanceFunction(distanceType);
    var match = 0;
    var minDistance = null;
    var k = measn.length;

    for(i = 0 ; i < k ; i++) {
        if(!minDistance || distance(means[i],data) < minDistance) {
            match = i;
            minDistance = distance(means[i],data);
        }
